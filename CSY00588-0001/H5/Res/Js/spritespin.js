...
(function($) {
  ...
  Spin.defaults = {
    ...
    reverse           : true,        // If true animation is played backward
  ...
  function load(opts){
    ...
    for (i = 0; i < src.length; i += 1 ) {
      ...
      img.src = src[i];
	 // var now=(i+1)/src.length;
	 // $(".jd").html(Math.round(now*100)+'%');
    }
  }
  ...
  Spin.sourceArray = function(path, opts){
    ...
    for (i = lStart; i <= lEnd; i+=1){
      for (j = fStart; j <= fEnd; j+=1){
		...
		if(temp[10]=="0")(temp=temp.substring(0,10)+temp.substring(11,temp.length));
        result.push(temp);
      }
    }
    return result;
